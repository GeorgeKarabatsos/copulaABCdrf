...
for (r in 1 : replicas) {  # r = 1
	...
	Nnodes 			= 	50
								sigma = -0.2	; 	tau = 0.5; 
	truth 				= 	c(	sigma			, 	tau			)
	parameterNames	=	c('kstar(2)'	, 	'triangles'	)
	...
	G 					= 	simulate(init.G ~ kstar(2) + triangles, nsim = 1, coef = truth)# in 'network' format
	#
	# Monte Carlo Maximum likelihood estimates and standard error:
	MCMLEout			=	ergm(formula = G ~ kstar(2) + triangles, estimate = "MLE", verbose = FALSE)
	MCMLE				=	coef(MCMLEout)
	...
	# g-prior setup:
	# ------------------------------------------------------------------------------------------
	invJ 				=	vcov(	ergm(	formula = G ~ kstar(2) + triangles, estimate = "MPLE"))
	...
	# ====================================================================================================================================
	outMPLEx			=	MPLEergm(G, c('kstar(2)', 'triangles'))
	...
	for (j in 1 : N) {
		...
		#init.G 		= 	as.network.matrix(init.G, directed = FALSE)
		Gtable			= 	simulate(init.G ~ kstar(2) + triangles, nsim = 1, coef = theta.j)# in 'network' format
	...
	seMPLEGodambe.simulations				=	rbind(seMPLEGodambe.simulations, 		seMPLEgodambe)
	...
	MPLE95Godambecover.simulations 		=	rbind(MPLE95Godambecover.simulations,	(truth > MPLEx -  (1.96 * seMPLEgodambe)) & (truth < MPLEx   + (1.96 * seMPLEgodambe)))
	...
	if (r == replicas){
		...
		Mean.seMPLEGodambe.simulations		=	apply(seMPLEGodambe.simulations, 2, mean, na.rm = T);		SD.seMPLEGodambe.simulations		=	apply(seMPLEGodambe.simulations, 2, sd, na.rm = T)
		...
		Prob.MPLE95Godambecover.simulations	=	apply(MPLE95Godambecover.simulations, 2, mean, na.rm = T)
	...
	outputFileName	=	paste("ERGM simul n = 50 NnodesTable = ", NnodesTable, " ", gsub("\\:", "_", round(end_time)), ".RData",sep='')
