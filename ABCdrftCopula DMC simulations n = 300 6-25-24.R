...
for (r in 1 : replicas) {	# r = 1		# For testing
	# ====================================================================================================================================
	# Simulate network dataset from DMC model (network dataset to be analyzed using ABC)
	# ====================================================================================================================================
	# Network data 
	# DMC model:
	Nnodes 			= 300 # Number of nodes in network (to simulate)
	parameterNames	=	c(	'q_m', 	'q_c')
	d				=	length(parameterNames)
	q_m = 0.2; 	q_c = 	0.1; # True parameters used by Chen & Onnela (2019, p. 5)
	truth 			= 	c(q_m, q_c); # True data-generating model parameters to be estimated.
	#	
	G 	= 	simulateNetwork(Nnodes = Nnodes, model = "DMC", parameters = truth)
	#
	...
	# ====================================================================================================================================
	outMPLEx		=	MPLEergm(G, c('meandeg', 'triangles')) # Used for DMC by Raynal et al. 2022,p.169
	MPLEx			=	outMPLEx$MPLE[1,]
	seMPLE			=	sqrt(outMPLEx$MPLE[2,] )
	MPLEterms		=	names(MPLEx)
	p 				=	length(MPLEterms)
	#
	...
	for (j in 1 : N) {	# j = 1		# For testing
		...
		# above, Uniform(0.15,0.35) prior for q_m used by Raynal et al.(2022,p.175), while they used a Uniform(0.1,0.9) prior for q_c.
		#
		# Simulate network dataset: 
		Gtable 		= 	simulateNetwork(Nnodes = NnodesTable, model = "DMC", parameters = theta.j)		
		#
		# Compute MPLE summary statistics of the simulated data:
		outMPLEy			=	MPLEergm(Gtable, c('meandeg', 'triangles'))
	...
	outputFileName	=	paste("DMC simul n = 300 NnodesTable = ", NnodesTable, " ", gsub("\\:", "_", round(end_time)), ".RData",sep='')
