...
for (r in 1 : replicas) {	# r = 1
	...
	k_0 = 1; 	p =	 .02;# Constraint 0.9 < k_0 < 1.1 will be applied in prior (Raynal etal.2022).
	...
	#	
	G 	= 	simulateNetwork(Nnodes = Nnodes, model = "Price", parameters = c(truth, alpha, maxOutdegree))
	#
	...
	# ====================================================================================================================================
	outMPLEx			=	MPLEergm(G, c('gwidegree','triangles'))
	...
	NnodesTable 	= 	floor(Nnodes * (1/3));  # Size of simulated network for References Table
	...
	for (j in 1 : N) {	# j = 1
		...
		theta.j		=	c(runif(1, min = 0.9, max = 1.1), runif(1, min = 0, max = .10))
		#
		...
		outMPLEy			=	MPLEergm(Gtable, c('gwidegree','triangles'))
	...
	priorPDFs			=	apply(cbind(dunif(theta.table.keep[,1], min = 0.9, max = 1.1), dunif(theta.table.keep[,2], min = 0, max = .10)),1,prod)
	...
	outputFileName	=	paste("Price simul NnodesTable = ", NnodesTable, " ", gsub("\\:", "_", round(end_time)), ".RData",sep='')
